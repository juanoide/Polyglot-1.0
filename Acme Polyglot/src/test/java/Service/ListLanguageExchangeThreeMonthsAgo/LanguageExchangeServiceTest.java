package Service.ListLanguageExchangeThreeMonthsAgo;
...
public class LanguageExchangeServiceTest {
	...
	// A user must be able list all language Exchange 3 months ago.

	// POSITIVE TEST CASE: An user can list all language Exchange 3 months ago.
	@Test
	public void testFindAllLanguageExchangeThreeMontsAgo() {
		Collection<LanguageExchange> LanguageExchanges = languageExchangeService.listExchangePastOrganised3Months();
	...
	Date date2=fecha2.getTime();
		for (LanguageExchange LanguageExchange : LanguageExchanges) {
			...
			res=res&&(LanguageExchange.getDate().before(fecha))&&(LanguageExchange.getDate().after(date2));
